...
namespace Feature.Wealth.Service.Repositories
{
    public class FundRankApiRepository
    {
        ...
        // 取得所有基金資料
        public IList<Funds> GetFundData(string data)
        {
        ...
        public List<ReturnFund> GetPerformanceFundRank(string dff, string data)
        {
            var funds = string.IsNullOrEmpty(dff)
                ? GetFundData(data).Take(10)
                : GetFundData(data).Where(f => f.DomesticForeignFundIndicator == dff).Take(10);
        ...
        {
            return JsonPostAsync().Select(fund => ConvertFormatFund(fund, data)).OrderByDescending(fund => fund.Tnav).ThenBy(fund => fund.FundName).ThenBy(fund => fund.FundCode).Take(10).ToList();
        }
