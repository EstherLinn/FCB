        ...
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                if (IsfilePath)
                {
                    try
                    {
                        var basic = await etlService.ParseCsv<SysjustReturnEtf2>(filename);
                        _repository.BulkInsertToDatabase(basic, "[Sysjust_Return_ETF_2_History]", "DataDate", "FirstBankCode", filename);
                        _repository.BulkInsertToNewDatabase(basic, "[Sysjust_Return_ETF_2]", filename);
