...
namespace Feature.Wealth.ScheduleAgent.Schedules.Sysjust
{
    public class InsertRiskFund2Domestic : SitecronAgentBase
    {
        ...
        public InsertRiskFund2Domestic()
        {
        ...
        protected override async Task Execute()
        {
            ...
            if (IsfilePath)
            {
                try
                {
                    var basic = await this._etlService.ParseCsv<SysjustRiskFund2Domestic>(filename);
                    _repository.BulkInsertDirectToDatabase(basic, "[Sysjust_Risk_Fund_2_Domestic_History]", filename);
