    ...
    public class InsertHoldingFund1 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                var scheduleName = ScheduleName.InsertHoldingFund1.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        _repository.BulkInsertToDatabase(datas, tableName + "_History", "FirstBankCode", "Date", "StockID", fileName, startTime, scheduleName, threadId);
