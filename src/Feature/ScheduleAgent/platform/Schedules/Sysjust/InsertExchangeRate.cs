        ...
        protected override async Task Execute()
        {
            ...
            if (this.JobItems != null)
            {
                ...
                if (IsfilePath)
                {
                    try
                    {
                        var basic = await etlService.ParseCsv<SysjustExchangeRate>(filename);
                        _repository.BulkInsertToNewDatabase(basic, "[Sysjust_ExchangeRate]", filename);
                        _repository.BulkInsertToDatabase(basic, "[Sysjust_ExchangeRate_History]", "CurrencyDescription", "CurrencyDescription", filename);
