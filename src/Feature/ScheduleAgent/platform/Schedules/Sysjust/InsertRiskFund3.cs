    ...
    public class InsertRiskFund3 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                string fileName = "SYSJUST-RISK-FUND-3";
                var TrafficLight = NameofTrafficLight.Sysjust_Risk_Fund_3;
                var scheduleName = ScheduleName.InsertRiskFund3.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<SysjustRiskFund3>(fileName);
                        ...
                        if (!Ischeck)
                        {
                            ...
                            _repository.BulkInsertToDatabase(datas, tableName + "_History", "FirstBankCode", "Date", fileName, startTime, scheduleName, threadId);
