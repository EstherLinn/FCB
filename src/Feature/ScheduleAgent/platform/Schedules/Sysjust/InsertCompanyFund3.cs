    ...
    public class InsertCompanyFund3 : SitecronAgentBase
    {
        ...
        protected override async Task Execute()
        {
            ...
            if (IsfilePath)
            {
                try
                {
                    var basic = await this._etlService.ParseCsv<SysjustCompanyFund3>(filename);
                    _repository.BulkInsertToDatabase(basic, "[Sysjust_Company_fund_3]", "IssuerCompanyID", "IssuerCompanyID", filename);
                }
                catch (Exception ex)
                {
