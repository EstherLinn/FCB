    ...
    public class InsertCompanyFund3 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                var scheduleName = ScheduleName.InsertCompanyFund3.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        _repository.BulkInsertToDatabase(datas, tableName + "_History", "IssuerCompanyID", "IssuerCompanyID", fileName, startTime, scheduleName, threadId);
