...
{
    public class InsertDividendFund : DefinedScheduleAgent
    {
        protected override async Task ExecuteJobItems()
        {
            if (this.JobItems != null)
            {
                ...
                {
                    StartTime = startTime,
                    ScheduleName = ScheduleName.InsertDividendFund.ToString(),
                    ThreadId = Thread.CurrentThread.ManagedThreadId,
                    TaskExecutionId = new ShortID(Guid.NewGuid()).ToString()
                };

                string fileName = "SYSJUST-DIVIDEND-FUND";
                var TrafficLight = NameofTrafficLight.Sysjust_Dividend_Fund;
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<SysjustDividendFund>(fileName);
                        ...
                        if (!Ischeck)
                        {
                            ...
                            _repository.BulkInsertToDatabaseForHIS(thirtyDaysData, tableName + "_History", "FirstBankCode", "ExDividendDate", fileName, context);
