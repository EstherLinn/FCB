    ...
    public class InsertFundSizeFund2 : SitecronAgentBase
    {
        ...
        protected override async Task Execute()
        {
            ...
            if (IsfilePath)
            {
                try
                {
                    var basic = await this._etlService.ParseCsv<SysjustFundSizeFund2>(filename);

                    this._repository.BulkInsertToDatabase(basic, "[Sysjust_Fundsize_Fund_2]", "ScaleDate", "FirstBankCode", filename);
