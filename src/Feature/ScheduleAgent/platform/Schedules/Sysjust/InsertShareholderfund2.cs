...
{
    public class InsertShareHolderFund2 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                string fileName = "SYSJUST-SHAREHOLDER-FUND-2";
                var TrafficLight = NameofTrafficLight.Sysjust_Shareholder_fund_2;
                var scheduleName = ScheduleName.InsertShareholderFund2.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<SysjustShareholderFund2>(fileName);
                        ...
                        _repository.BulkInsertToDatabase(datas, tableName + "_History", "FundCompanyCode", "Name", "Title", fileName, startTime, scheduleName, threadId);
