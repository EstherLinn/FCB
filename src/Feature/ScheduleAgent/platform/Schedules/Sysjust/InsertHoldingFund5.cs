    ...
    public class InsertHoldingFund5 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                var scheduleName = ScheduleName.InsertHoldingFund5.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        if (!Ischeck)
                        {
                            ...
                            _repository.BulkInsertToDatabase(datas, tableName + "_History", "FirstBankCode", "Date", "StockCode", fileName, startTime, scheduleName, threadId);
