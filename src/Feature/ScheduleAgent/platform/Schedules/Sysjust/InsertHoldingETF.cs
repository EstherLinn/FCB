    ...
    public class InsertHoldingEtf : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                var scheduleName = ScheduleName.InsertHoldingEtf1.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        if (!Ischeck)
                        {
                            ...
                            _repository.BulkInsertToDatabase(datas, tableName + "_History", "FirstBankCode", "Date", "IndustryName", fileName, startTime, scheduleName, threadId);
