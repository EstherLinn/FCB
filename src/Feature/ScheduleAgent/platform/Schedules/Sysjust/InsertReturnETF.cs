        ...
        protected override async Task Execute()
        {
            ...
            if (this.JobItems != null)
            {
                ...
                if (IsfilePath)
                {
                    try
                    {
                        var basic = await etlService.ParseCsv<SysjustReturnEtf>(filename);
                        _repository.BulkInsertToDatabase(basic, "[Sysjust_Return_ETF_History]", "FirstBankCode", "NetAssetValueDate", filename);
