    ...
    public class InsertRiskEtf2 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                var TrafficLight = NameofTrafficLight.Sysjust_Risk_ETF_2;
                var scheduleName = ScheduleName.InsertRiskEtf2.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<SysjustRiskEtf2>(fileName);
