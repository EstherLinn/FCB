    ...
    public class InsertHoldingFund3 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                string fileName = "SYSJUST-HOLDING-FUND-3";
                var TrafficLight = NameofTrafficLight.Sysjust_Holding_Fund_3;
                var scheduleName = ScheduleName.InsertHoldingFund3.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<SysjustHoldingFund3>(fileName);
                        ...
                        if (!Ischeck)
                        {
                            ...
                            _repository.BulkInsertToDatabase(datas, tableName + "_History", "FirstBankCode", "Date", "Sector", fileName, startTime, scheduleName, threadId);
