...
{
    public class InsertHoldingFund3 : DefinedScheduleAgent
    {
        protected override async Task ExecuteJobItems()
        {
            if (this.JobItems != null)
            {
                ...
                {
                    StartTime = startTime,
                    ScheduleName = ScheduleName.InsertHoldingFund3.ToString(),
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        if (!Ischeck)
                        {
                            ...
                            _repository.BulkInsertToDatabase(datas, tableName + "_History", "FirstBankCode", "Date", "Sector", fileName, context);
