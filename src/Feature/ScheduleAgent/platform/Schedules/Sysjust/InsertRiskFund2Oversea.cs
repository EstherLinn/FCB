    ...
    public class InsertRiskFund2Oversea : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                var TrafficLight = NameofTrafficLight.Sysjust_Risk_Fund_2_Oversea;
                var scheduleName = ScheduleName.InsertRiskFund2Oversea.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<SysjustRiskFund2Oversea>(fileName);
