    ...
    public class InsertReturnFund3 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                string fileName = "SYSJUST-RETURN-FUND-3";
                var TrafficLight = NameofTrafficLight.Sysjust_Return_Fund_3;
                var scheduleName = ScheduleName.InsertReturnFund3.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<SysjustReturnFund3>(fileName);
