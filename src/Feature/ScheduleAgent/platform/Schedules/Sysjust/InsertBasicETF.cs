...
{
    public class InsertBasicEtf : DefinedScheduleAgent
    {
        protected override async Task ExecuteJobItems()
        {
            if (this.JobItems != null)
            {
                ...
                {
                    StartTime = startTime,
                    ScheduleName = ScheduleName.InsertBasicEtf.ToString(),
                ...
                if (isFilePath.Value)
                {
                    try
                    {
                        ...
                        var Ischeck = _repository.CheckDataCount(tableName, fileName, count, context);
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _repository.LogChangeHistory(fileName, ex.Message, string.Empty, 0, executionTime, "N", ModificationID.Error, context);
                    }
                }
                else
                {
                    ...
                    var executionTime = (DateTime.UtcNow - startTime).TotalSeconds;
                    _repository.LogChangeHistory(fileName, isFilePath.Key, string.Empty, 0, executionTime, "N", ModificationID.Error, context);
                }
