    ...
    public class InsertHoldingEtf2 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                var scheduleName = ScheduleName.InsertHoldingEtf2.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        _repository.BulkInsertToDatabase(datas, tableName + "_History", "FirstBankCode", "RegionName", "Date", fileName, startTime, scheduleName, threadId);
