...
{
    public class InsertHoldingEtf2 : DefinedScheduleAgent
    {
        protected override async Task ExecuteJobItems()
        {
            if (this.JobItems != null)
            {
                ...
                {
                    StartTime = startTime,
                    ScheduleName = ScheduleName.InsertHoldingEtf2.ToString(),
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        if (!Ischeck)
                        {
                            ...
                            _repository.BulkInsertToDatabase(datas, tableName + "_History", "FirstBankCode", "RegionName", "Date", fileName, context);
