    ...
    public class InsertRiskFund4 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                var TrafficLight = NameofTrafficLight.Sysjust_Risk_Fund_4;
                var scheduleName = ScheduleName.InsertRiskFund4.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<SysjustRiskFund4>(fileName);
