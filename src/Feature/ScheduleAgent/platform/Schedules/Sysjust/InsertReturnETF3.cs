    ...
    public class InsertReturnEtf3 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                var TrafficLight = NameofTrafficLight.Sysjust_Return_ETF_3;
                var scheduleName = ScheduleName.InsertReturnEtf3.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<SysjustReturnEtf3>(fileName);
                        ...
                        _repository.BulkInsertToDatabase(datas, tableName + "_History", "FirstBankCode", "DataDate", fileName, startTime, scheduleName);
