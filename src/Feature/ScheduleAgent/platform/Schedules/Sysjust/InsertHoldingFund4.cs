...
{
    public class InsertHoldingFund4 : DefinedScheduleAgent
    {
        protected override async Task ExecuteJobItems()
        {
            if (this.JobItems != null)
            {
                ...
                {
                    StartTime = startTime,
                    ScheduleName = ScheduleName.InsertHoldingFund4.ToString(),
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        if (!Ischeck)
                        {
                            ...
                            _repository.BulkInsertToDatabase(datas, tableName + "_History", "FirstBankCode", "Date", "StockName", fileName, context);
