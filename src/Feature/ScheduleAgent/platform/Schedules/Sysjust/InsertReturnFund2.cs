    ...
    public class InsertReturnFund2 : SitecronAgentBase
    {
        ...
        protected override async Task Execute()
        {
            ...
            if (IsfilePath)
            {
                try
                {
                    var basic = await this._etlService.ParseCsv<SysjustReturnFund2>(filename);
                    _repository.BulkInsertToDatabase(basic, "[Sysjust_Return_Fund_2]", "Year", "FirstBankCode", filename);
                }
                catch (Exception ex)
                {
