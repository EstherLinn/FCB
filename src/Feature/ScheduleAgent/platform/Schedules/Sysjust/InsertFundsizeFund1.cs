    ...
    public class InsertFundSizeFund1 : SitecronAgentBase
    {
        ...
        protected override async Task Execute()
        {
            ...
            if (IsfilePath)
            {
                try
                {
                    var basic = await this._etlService.ParseCsv<SysjustFundSizeFund1>(filename);

                    _repository.BulkInsertToDatabase(basic, "[Sysjust_Fundsize_Fund_1]", "ScaleDate", "FirstBankCode", filename);
                }
                catch (Exception ex)
                {
