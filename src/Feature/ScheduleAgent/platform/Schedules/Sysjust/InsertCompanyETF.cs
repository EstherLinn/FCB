        ...
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                if (IsfilePath)
                {
                    try
                    {
                        var basic = await etlService.ParseCsv<SysjustCompanyEtf>(filename);
                        _repository.BulkInsertToNewDatabase(basic, "[Sysjust_Company_ETF]", filename);
                        _repository.BulkInsertToDatabase(basic, "[Sysjust_Company_ETF_History]", "IssueCompanyID", "IssueCompanyID", filename);
