...
{
    public class InsertShareHolderFund1 : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                string fileName = "SYSJUST-SHAREHOLDER-FUND-1";
                var TrafficLight = NameofTrafficLight.Sysjust_Shareholder_fund_1;
                var scheduleName = ScheduleName.InsertShareholderFund1.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<SysjustShareholderFund1>(fileName);
                        ...
                        if (!Ischeck)
                        {
                            ...
                            _repository.BulkInsertToDatabase(datas, tableName + "_History", "FundCompanyCode", "ShareholderName", fileName, startTime, scheduleName, threadId);
