...
{
    public class InsertFundTrb : DefinedScheduleAgent
    {
        protected override async Task ExecuteJobItems()
        {
            if (this.JobItems != null)
            {
                ...
                {
                    StartTime = startTime,
                    ScheduleName = ScheduleName.InsertFundTrb.ToString(),
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        _repository.BulkInsertToDatabase(datas, tableName, "ProductCode", "ProductCode", fileName, context);
