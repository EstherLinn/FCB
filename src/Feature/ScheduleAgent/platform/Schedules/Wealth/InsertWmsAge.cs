...
{
    public class InsertWmsAge : DefinedScheduleAgent
    {
        protected override async Task ExecuteJobItems()
        {
            if (this.JobItems != null)
            {
                ...
                {
                    StartTime = startTime,
                    ScheduleName = ScheduleName.InsertWmsAge.ToString(),
                    ThreadId = Thread.CurrentThread.ManagedThreadId,
                    TaskExecutionId = new ShortID(Guid.NewGuid()).ToString()
                };

                string fileName = "wms_age_profile_d_mf";
                var TrafficLight = NameofTrafficLight.Wms_age_profile_d_mf;
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<Wmsageprofiledmf>(fileName);
