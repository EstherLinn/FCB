...
{
    public class InsertWmsZodiac : DefinedScheduleAgent
    {
        protected override async Task ExecuteJobItems()
        {
            if (this.JobItems != null)
            {
                ...
                {
                    StartTime = startTime,
                    ScheduleName = ScheduleName.InsertWmsZodiac.ToString(),
                    ThreadId = Thread.CurrentThread.ManagedThreadId,
                    TaskExecutionId = new ShortID(Guid.NewGuid()).ToString()
                };

                string fileName = "wms_zodiac_profile_d_mf";
                var TrafficLight = NameofTrafficLight.Wms_zodiac_profile_d_mf;
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<Wmszodiacprofiledmf>(fileName);
