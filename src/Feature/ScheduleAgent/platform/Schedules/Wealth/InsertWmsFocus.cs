...
{
    public class InsertWmsFocus : SitecronAgentBase
    {
        protected override async Task Execute()
        {
            if (this.JobItems != null)
            {
                ...
                string fileName = "wms_focus_profile";
                var TrafficLight = NameofTrafficLight.Wms_focus_profile;
                var scheduleName = ScheduleName.InsertWmsFocus.ToString();
                ...
                if (IsfilePath.Value)
                {
                    try
                    {
                        ...
                        var datas = await etlService.ParseCsv<Wmsfocusprofile>(fileName);
