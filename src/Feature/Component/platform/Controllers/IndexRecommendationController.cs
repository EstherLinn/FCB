    ...
    public class IndexRecommendationController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var viewModel = new IndexRecommendationModel
            {
                ...
                USStockDataDate = usStocks.Where(f => string.IsNullOrEmpty(f.DataDate) && DateTime.TryParse(f.DataDate, out var d)).Select(f => DateTime.Parse(f.DataDate)).FirstOrDefault(),
                BondDataDate = bonds.Where(f => string.IsNullOrEmpty(f.Date) && DateTime.TryParse(f.Date, out var d)).Select(f => DateTime.Parse(f.Date)).FirstOrDefault(),
            };
